...
class Graph(Scene):
    ...
    def intro(self):
        text_intro = TextMobject("图论总结（一）", color=RED).scale(2.25).shift(UP * 0.5)
        text_producer = TextMobject("By Talaodi", color=YELLOW).shift(DOWN * 1.25).scale(1.25)
        text_copy_right = TextMobject("使用 Python + Manim 制作").move_to(BOTTOM).scale(0.8).shift(0.6 * UP)
    ...
    def graph(self):
        ...
        text_G_nodes = TexMobject("Nodes").scale(3).next_to(text_G[0][1]).shift(2 * LEFT)
        text_G_edges = TexMobject("Edges").scale(3)
        G_real_nodes = self.nodes.copy().scale(0.125).next_to(text_G[0][1]).shift(2 * LEFT)
        G_real_edges = self.edges.copy().scale(0.125)

        self.play(Write(text_G))
        self.wait(1)

        self.play(text_G[0][3:].next_to, text_G_nodes, text_G[0][0:2].next_to, text_G_nodes, LEFT, Transform(text_G[0][2], text_G_nodes)) 
        self.wait(1)

        self.play(text_G[0][3:].next_to, G_real_nodes, text_G[0][0:2].next_to, G_real_nodes, LEFT, Transform(text_G[0][2], G_real_nodes))
        self.wait(1)
        
        text_G_edges.next_to(text_G[0][3], aligned_edge=DOWN)
        self.play(text_G[0][5:].next_to, text_G_edges, Transform(text_G[0][4], text_G_edges))
        self.wait(1)

        G_real_edges.move_to(text_G_edges)
        self.play(Transform(text_G[0][4], G_real_edges), text_G[0][5:].next_to, G_real_edges, text_G[0][:4].next_to, G_real_edges, LEFT)
        self.wait(1)

        self.play(FadeOut(VGroup(text_G[0][1], text_G[0][3], text_G[0][5])), 
                  Transform(text_G[0][2], self.nodes.copy().scale(0.875).shift(0.775 * UP)), 
                  Transform(text_G[0][4], self.edges.copy().scale(0.875).shift(0.775 * UP)),
        ...
        self.wait(3)
        self.play(FadeOut(VGroup(self.nodes_after_graph, self.edges_after_graph, text_G[0][0])))
    ...
    def top(self):
        ...
        def add_and_delete(self, index_of_node, last_text, new_com, new, new_line=False):
            ...
            if new:
                self.play(FadeOut(VGroup(*edges_make_non)), node.move_to, BOTTOM + 0.75 * UP + 3.5 * LEFT)
                ...
                if not new_line:
                    ...
                    self.play(FadeOut(VGroup(*edges_make_non)), FadeIn(com), node.move_to, temp_node)
                else:
                    com.next_to(last_text, aligned_edge=DOWN)
                    self.play(FadeOut(VGroup(*edges_make_non)), FadeIn(com), node.next_to, last_text[0], DOWN, aligned_edge=LEFT)
        ...
        for i in range(1, 6):
            text_top = add_and_delete(self, topsort[i] - 1, text_top, True, False)

        self.play(VGroup(*(nodes[topsort[i] - 1] for i in range(6, 12))).shift, UP, VGroup(*(edges[i] for i in range(15) if not used[i])).shift, UP, text_top.shift, UP)
        text_top = add_and_delete(self, topsort[6] - 1, text_top, True, False, True)
        ...
        text_topsort = TextMobject("Topological-sort").scale(2.5).next_to(text_top, DOWN).shift(DOWN)
    ...
    def prim(self):
        ...
        make_arrow = lambda s, t: DoubleArrow(self.nodes[s - 1][0].get_arc_center(), self.nodes[t - 1][0].get_arc_center(), 
        ...
        text_start = TextMobject("start: 1").next_to(graph, DOWN, aligned_edge=LEFT).shift(2 * LEFT + 0.5 * DOWN).scale(1.25)
        ...
        self.wait(1)
        self.play(nodes[0][0].set_color, BLUE, form[0].set_fill, {"opacity": 0.6}, form[0].set_fill, GRAY)
        last_node = 0
        used = [True] + 11 * [False]
        choosed = [False] * 15
        ...
        def set_color_and_add():
            ...
            index_of_edge = (i for i in range(15) if last_node + 1 in self.edges_number[i][:2])
            trans = ([], [])
            light = list()
            for i in index_of_edge:
                ...
                if (not used[to - 1]):
                    ...
                    if text_list[to - 1] == "$\infty$":
                        ...
                        trans[1].append(TexMobject(text_list[to - 1]).move_to(form[to - 1][1]))
            ...
            transform  = None
            make_edge = lambda s, t: Arrow(nodes[s - 1][0].get_arc_center(), nodes[t - 1][0].get_arc_center(), 
        ...
        text_prim = TextMobject("$Prim$(to find the $MST$)").scale(2).move_to(form)
    ...
    def dijkstra(self):
        nodes = self.nodes.copy()
        make_arrow = lambda s, t: Arrow(self.nodes[s - 1][0].get_arc_center(), self.nodes[t - 1][0].get_arc_center(), 
                           buff=0.5, stroke_width=3, max_tip_length_to_length_ratio=0.15)
        edges = VGroup(*(VGroup(make_arrow(self.edges_number[i][0], self.edges_number[i][1]).set_opacity(0.6), 
                                TexMobject("%d" % self.edges_number[i][2], color=ORANGE).move_to(self.edges.copy()[i])) for i in range(15)))
        ...
        def set_color_and_add():
            ...
            index_of_edge = (i for i in range(15) if last_node + 1 == self.edges_number[i][0])
            trans = ([], [])
            light = list()
            for i in index_of_edge:
                edge = self.edges_number[i]
                ...
                if (not used[to - 1]):
                    ...
                    if text_list[to - 1] == "$\infty$":
                        father[to - 1] = ptr
                        text_list[to - 1] = str(int(text_list[ptr - 1]) + value)
                        trans[0].append(form[to - 1][1])
                        trans[1].append(TexMobject(text_list[to - 1]).move_to(form[to - 1][1]))
            ...
            self.play(Transform(VGroup(*trans[0]), VGroup(*trans[1])))
            self.play(VGroup(*(light)).set_color, WHITE, 
                      VGroup(*(edges[i][0] for i in set_color_edges_index)).set_color, BLUE,
                      nodes[index_of_node][0].set_color, BLUE, 
                      form[last_node].set_color, WHITE,
                      form[index_of_node].set_color, YELLOW,
                      form[index_of_node].set_fill, {"opacity": 0.6},
            ...
            if set_color_edges_index:
                self.wait(1)
                self.play(VGroup(*(edges[i] for i in set_color_edges_index)).set_color, WHITE)
        ...
        for i in range(1, 12):
            if used[i]:
                ...
                self.play(VGroup(*(edges[i][0] for i in paths[i])).set_color, BLUE)
                self.wait(1)
                self.play(form[i].set_color, WHITE, VGroup(*(edges[i][0] for i in paths[i])).set_color, WHITE)
                self.wait(1)

        self.wait(1)
        text_dij = TextMobject("$Dijkstra$(to find the $SP$)").scale(2).move_to(form)
    ...
    def make_graph(self):
        nodes = list()
        make_nodes = lambda u, l, i: VGroup(Circle().shift(u * UP + l * LEFT).scale(0.4), TextMobject("%d" % i, color=YELLOW).shift(u * UP + l * LEFT))
        ...
        edges = list()
        make_edges = lambda s, t: Arrow(nodes[s - 1][0].get_arc_center(), nodes[t - 1][0].get_arc_center(), 
                                        buff=0.5, stroke_width=3, max_tip_length_to_length_ratio=0.15)
