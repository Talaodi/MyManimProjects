...
class Integral(GraphScene):
    ...
    def construct(self):
        ...
        start_line = Line(self.input_to_graph_point(
            1, self.square_graph), self.coords_to_point(1, 0), color=YELLOW)
        end_line = Line(self.input_to_graph_point(
            3, self.square_graph), self.coords_to_point(3, 0), color=YELLOW)
        ...
        text_number = TextMobject("Number:1").next_to(
            VGroup(text_start, text_end), DOWN)
        ...
        for number in numbers:
            ...
            text_new_number = TexMobject(
                str(number)).next_to(text_number[0][6], RIGHT)
            text_new_dx = TexMobject(
                "%.2f" % (2 / number)).next_to(text_dx[0][2], RIGHT)
            self.play(
                Transform(last, now),
        ...
        self.wait(1)
        text_sum = TexMobject("\\sum_i f(x_i){\\rm d}x \\approx %.2f" % (
            9 - 0.33)).next_to(text_dx, DOWN)
        text_int = TexMobject(
            "\\int_1^3 f(x){\\rm d}x=\\frac{26}{3}").move_to(text_sum)
    ...
    def make_rectangles(self, start, end, number=1):
        width = (self.coords_to_point(end, 0)[
                 0] - self.coords_to_point(start, 0)[0]) / number
